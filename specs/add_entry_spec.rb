...
describe "test_all" do
    ...
    describe "add_entry_specs" do
        it "demonstrates that license_plate_valid function is being invoked in add_entry_call_methods function" do
            expect(parking_lot).to receive(:license_plate_valid).with("KA02KB0001")

            parking_lot.add_entry_call_methods("KA02KB0001")
        end

        it "demonstrates that duplicates_plates_check function is being invoked in add_entry_call_methods function" do
            expect(parking_lot).to receive(:duplicates_plates_check).with("KA02KB0002")
