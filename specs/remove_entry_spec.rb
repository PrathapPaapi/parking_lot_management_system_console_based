...
describe "test_all" do
    ...
    describe "remove_entry_specs" do
        ...
        it "demonstrates that license_plate_valid function is being invoked in remove_entry_function function" do
            expect(parking_lot).to receive(:license_plate_valid).with("KA02KB0001")

            parking_lot.remove_entry_function("KA02KB0001")
        end

        it "demonstrates that check_license_plate function is being invoked in remove_entry_function function" do
            expect(parking_lot).to receive(:check_license_plate).with("KA02KB0001")
        ...
        it "demonstrates that generate_invoice function is being invoked in remove_entry_function function" do
            expect(parking_lot).to receive(:generate_invoice).with(parking_lot.check_license_plate("KA02KB0001"))

            parking_lot.remove_entry_function("KA02KB0001")
        end

        it "demonstrates that remove_from_entry_array function removes the entry from entries array" do
            entry = parking_lot.check_license_plate("KA02KB0001")
            new_array = [entry]
